...
def api_search():
    ...
    if session.get("user_id"):
        ...
        form_selection.selection_list.choices = [
            (x.id, x.name) for x in user.user_selection
        ]
    ...
    if form_word.validate_on_submit():
        ...
        api_request = requests.get(
            "https://api.dictionaryapi.dev/api/v2/entries/en/" + user_input
        )
        ...
        return render_template(
            "home.html", form=form_word, answer=answer, form_selection=form_selection
        )
